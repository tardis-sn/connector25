...
MESA-explosion can provide profiles which include fall back
...
The fallback is implemented through a no-gradients inner
boundary and taking `v(1)` to be the minimum between `0` and `v(2)` if
the latter is negative.

Hereafter, we interchangeably refer to `SNEC` or `SNEC+fb`, unless
specified otherwise.
...
If everything is in the right place, compile with `make` and type
`./snec`!
