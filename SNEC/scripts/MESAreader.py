...
def get_model_initial_values(model):
    """ read initial conditions from folder, to avoid rounding issues """
    mass_string, omega_string  = model.split('/')[-2].split('_')
    mass = float(mass_string.split('/')[-1])
    omega = float(omega_string.replace('rot', ''))
    return(mass, omega)
