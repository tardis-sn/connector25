program main
   ...
   !my_mesa_dir = '/home/aurimontem/mesa-r11701' !'../..'
   !my_mesa_dir = '/Users/Benny/Documents/Research/MESA/mesa-r11701' !'../..'
   my_mesa_dir = '/home/bthtsang/Research/MESA/mesa-r11701' 
   ...
   do j=1,num_logRhos
      do i=1,num_logTs
         ...
         logRho = log10_cr(density)
         logRhos(j) = logRho
         logT = log10_cr(temperature)
         if (j == 1) then
            Ts(i) = temperature
            logTs(i) = logT
...
335 continue
   ...
   do nm=1,num_models
      ...
      if (sum_tau >= 1d0+tauph) then ! record photosphere information
         do j=zone,2,-1
            if(tau(j-1,nm) >= tauph) then
               ...
               log_g = log10_cr(standard_cgrav*mphot*Msun/(rphot*rphot))
               logT = log10_cr(alfa*temp(j,nm)+beta*temp(j-1,nm))
               Xsurf = alfa*h(j)+beta*h(j-1)
               Ysurf = alfa*he(j)+beta*he(j-1)
               Zsurf = max(1d-99, min(1d0, 1d0 - (Xsurf + Ysurf)))
               Z_div_X = Zsurf/max(1d-99, Xsurf)
               Fe_H = log10_cr(Z_div_X/Z_div_X_solar)
      ...
      do k=zone-1,2,-1
         ...
         ! Update fe composition after radioactive decay
         call decay_ni_composition(time, fe(k), ni(k), fe_final)
         n_Fe = density*avo*fe_final/A_Fe56
...
333 continue
   ...
   call save_day_post_Lbol_max(12d0,t0,zone,star_mass,mass_IB,'012')
   call save_day_post_Lbol_max(14d0,t0,zone,star_mass,mass_IB,'014')
   call save_day_post_Lbol_max(16d0,t0,zone,star_mass,mass_IB,'016')
   call save_day_post_Lbol_max(18d0,t0,zone,star_mass,mass_IB,'018')
   call save_day_post_Lbol_max(20d0,t0,zone,star_mass,mass_IB,'020')
   call save_day_post_Lbol_max(22d0,t0,zone,star_mass,mass_IB,'022')
   call save_day_post_Lbol_max(24d0,t0,zone,star_mass,mass_IB,'024')
   call save_day_post_Lbol_max(26d0,t0,zone,star_mass,mass_IB,'026')
   call save_day_post_Lbol_max(28d0,t0,zone,star_mass,mass_IB,'028')
   ...
   call save_day_post_Lbol_max(92d0,t0,zone,star_mass,mass_IB,'092')
   call save_day_post_Lbol_max(94d0,t0,zone,star_mass,mass_IB,'094')
   call save_day_post_Lbol_max(96d0,t0,zone,star_mass,mass_IB,'096')
   call save_day_post_Lbol_max(98d0,t0,zone,star_mass,mass_IB,'098')
   call save_day_post_Lbol_max(100d0,t0,zone,star_mass,mass_IB,'100')
   call save_day_post_Lbol_max(103d0,t0,zone,star_mass,mass_IB,'103')
   call save_day_post_Lbol_max(106d0,t0,zone,star_mass,mass_IB,'106')
   call save_day_post_Lbol_max(110d0,t0,zone,star_mass,mass_IB,'110')
   call save_day_post_Lbol_max(113d0,t0,zone,star_mass,mass_IB,'113')
   call save_day_post_Lbol_max(116d0,t0,zone,star_mass,mass_IB,'116')
   call save_day_post_Lbol_max(120d0,t0,zone,star_mass,mass_IB,'120')
   call save_day_post_Lbol_max(123d0,t0,zone,star_mass,mass_IB,'123')
   call save_day_post_Lbol_max(126d0,t0,zone,star_mass,mass_IB,'126')
...
333    continue
   ...
   subroutine decay_ni_composition(day, fe_init, ni_init, fe_final)
     real(dp), intent(in) :: day, fe_init, ni_init
     real(dp), intent(out) :: fe_final

     ! Radioactive decay constants, taken from Sedona's header.
     real(dp), parameter :: tau_56ni = 8.76437 ! in days
     real(dp), parameter :: tau_56co = 111.428 ! in days

     real(dp) :: co_init    ! placeholder for initial Co abundance
     co_init = 0.0  ! by default is zero, just in case we have non-zero initial
                    ! co abundance later

     real(dp) :: ni_f, co_f, fe_f, eco
     real(dp) :: ni_final, co_final

     ni_f = exp(-day/tau_56ni)
     co_f = tau_56co/(tau_56ni - tau_56co)*exp(-day/tau_56ni)&
           - exp(-day/tau_56co)
     fe_f = 1.0 - ni_f - co_f
     eco_f = exp(-day/tau_56co)

     ! not in output, left here for completeness
     ni_final = ni_init * ni_f
     co_final = ni_init * co_f + co_init*eco

     ! Updated Fe composition of a zone
     fe_final = fe_init + ni_init*fe_f + co_init*(1.0 - eco)
     
   end subroutine decay_ni_composition
